...
public class ReplyCommand {
    ...
    public void execute(CommandArgs args) {
        ...
        if(args.length() == 0) {
            sender.sendMessage(CC.translate(CobraPlugin.get().getConfig().getString("command.message.reply.usage"))
            ...
            if(TogglePmCommand.togglepm.contains(player) || TogglePmCommand.togglepm.contains(MessageCommand.target)) {
                sender.sendMessage(CC.translate(CobraPlugin.get().getConfig().getString("command.message.togglepm.messages_disabled")));
            } else {
                CobraPlugin.get().getConfig().getStringList("command.message.to").forEach(string -> sender.sendMessage(CC.translate(string
