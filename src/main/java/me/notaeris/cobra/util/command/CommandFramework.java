package me.notaeris.cobra.util.command;
        ...
        for (int i = args.length; i >= 0; i--) {
            ...
            if (commandMap.containsKey(cmdLabel)) {
                ...
                if (command.playerOnly() && !(sender instanceof Player)) {
                    sender.sendMessage(ChatColor.translateAlternateColorCodes('&', CobraPlugin.get().getConfig().getString("executor")));
                    return true;
                }
                if (!command.permission().equals("") && !sender.hasPermission(command.permission())) {
                    sender.sendMessage(ChatColor.translateAlternateColorCodes('&', CobraPlugin.get().getConfig().getString("no_permission")));
                    return true;
                }
                try {
